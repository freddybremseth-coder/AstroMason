...
// --- Start of inlined content from services/tarot-prompts.ts ---
...
export default async (req, res) => {
    ...
    try {
        ...
        if (error instanceof Anthropic.APIError) {
             if (error.status === 401) {
                return res.status(401).json({
                    error: {
                        message: 'Autentisering feilet. API-nøkkelen er sannsynligvis ugyldig.',
              ...
              if (error.status === 429) {
                return res.status(429).json({
                    error: {
                        message: 'Du har overskredet din kvote hos AI-tjenesten. Prøv igjen senere.',
                        details: error.message
                    }
                });
             }
        ...
        res.status(500).json({
            error: {
                message: 'En intern feil oppstod under kommunikasjon med AI-tjenesten.',
